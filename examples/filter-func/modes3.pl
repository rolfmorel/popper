%% (base) ➜  filter-func popp exs.pl modes3.pl bk.pl
%% f(A,B) :- empty(A),empty(B).
%% f(A,B) :- cons2(C,D,A),odd(C),f(D,B).
%% f(A,B) :- even(E),cons2(E,D,A),f(D,C),cons1(E,C,B).
%% python3 /Users/andrew/icloud/code/popper/popper.py exs.pl modes3.pl bk.pl  153.19s user 0.38s system 100% cpu 2:33.56 total
